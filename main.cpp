...
int main(int argc, char *argv[])
{
	...
	MPI_Scatter(img.data, 3 * image_cols * image_rows, MPI_UNSIGNED_CHAR,
							image, 3 * image_cols * image_rows, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);

	MPI_Scatter(gry_img.data, image_cols * image_rows, MPI_UNSIGNED_CHAR,
							gray_image, image_cols * image_rows, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);

	to_grayscale(image, gray_image, image_cols * image_rows, local_histogram);

	MPI_Gather(image, 3 * image_cols * image_rows, MPI_UNSIGNED_CHAR,
						 img.data, 3 * image_cols * image_rows, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);

	MPI_Reduce(local_histogram, global_histogram, 256, MPI_INT,
	...
	MPI_Gather(gray_image, image_cols * image_rows, MPI_UNSIGNED_CHAR,
						 gry_img.data, image_cols * image_rows, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
	...
	if (rank == 0)
	{
		...
		write = end - begin;
		total = read + gray + thrshld + binary + write;
	}
	...
	// 		// be careful with integer overflow in high resolutions (> 8k)
	// 		begin = omp_get_wtime();
	// 		to_grayscale(img.data, gry_img.data, img.rows * img.cols, histogram, threads);
	// 		end = omp_get_wtime();
	...
	// 		begin = omp_get_wtime();
	// 		to_binary(gry_img.data, img.rows * img.cols, t, threads);
	// 		end = omp_get_wtime();
